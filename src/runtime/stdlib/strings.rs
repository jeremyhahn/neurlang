...
pub fn register_string_extensions(registry: &mut ExtensionRegistry) {
    // string_new() -> handle
    registry.register(
        "string_new",
    ...
    // string_from_bytes(ptr, len) -> handle
    registry.register(
        "string_from_bytes",
    ...
    // string_concat(handle1, handle2) -> new_handle
    registry.register(
        "string_concat",
    ...
    // string_substr(handle, start, len) -> new_handle
    registry.register(
        "string_substr",
    ...
    // string_find(handle, needle_handle) -> index or -1
    registry.register(
        "string_find",
    ...
    // string_replace(handle, pattern, replacement) -> new_handle
    registry.register(
        "string_replace",
    ...
    // string_split(handle, delimiter) -> vec_handle
    registry.register(
        "string_split",
        "Split string by delimiter. Args: handle, delimiter_handle. Returns Vec handle of string handles.",
        2,
        true,
        ExtCategory::Strings,
        Arc::new(|args, outputs| {
    ...
    // string_to_upper(handle) -> new_handle
    registry.register(
        "string_to_upper",
    ...
    // string_to_lower(handle) -> new_handle
    registry.register(
        "string_to_lower",
    ...
    // string_starts_with(handle, prefix) -> 1 or 0
    registry.register(
        "string_starts_with",
    ...
    // string_ends_with(handle, suffix) -> 1 or 0
    registry.register(
        "string_ends_with",
    ...
    // string_to_bytes(handle, ptr, max_len) -> bytes_written
    registry.register(
        "string_to_bytes",
    ...
    // string_parse_float(handle) -> value (as bits)
    registry.register(
        "string_parse_float",
    ...
    // string_from_float(value_bits) -> handle
    registry.register(
        "string_from_float",
...
mod tests {
    ...
    fn test_string_operations() {
        ...
        // Trim (no change expected)
        let trimmed = string_trim_impl(handle).unwrap();
        assert_eq!(string_len_impl(trimmed).unwrap(), 13);

        // To upper
        let upper = string_to_upper_impl(handle).unwrap();
        let storage = STRING_STORAGE.read().unwrap();
        assert_eq!(storage.get(&upper).unwrap(), "HELLO, WORLD!");

        // To lower
        drop(storage);
        let lower = string_to_lower_impl(handle).unwrap();
        let storage = STRING_STORAGE.read().unwrap();
        assert_eq!(storage.get(&lower).unwrap(), "hello, world!");

        // Free
        drop(storage);
        string_free_impl(handle).unwrap();
        string_free_impl(upper).unwrap();
        string_free_impl(lower).unwrap();
        string_free_impl(trimmed).unwrap();
    }

    #[test]
    fn test_string_concat() {
        let h1 = string_from_bytes_impl(b"Hello, ".as_ptr(), 7).unwrap();
        let h2 = string_from_bytes_impl(b"World!".as_ptr(), 6).unwrap();

        let concat = string_concat_impl(h1, h2).unwrap();

        let storage = STRING_STORAGE.read().unwrap();
        assert_eq!(storage.get(&concat).unwrap(), "Hello, World!");
    }

    #[test]
    fn test_string_find() {
        let haystack = string_from_bytes_impl(b"Hello, World!".as_ptr(), 13).unwrap();
        let needle = string_from_bytes_impl(b"World".as_ptr(), 5).unwrap();
        let not_found = string_from_bytes_impl(b"xyz".as_ptr(), 3).unwrap();

        assert_eq!(string_find_impl(haystack, needle).unwrap(), 7);
        assert_eq!(string_find_impl(haystack, not_found).unwrap(), -1);
    }

    #[test]
    fn test_string_parse() {
        let int_str = string_from_bytes_impl(b"12345".as_ptr(), 5).unwrap();
        assert_eq!(string_parse_int_impl(int_str).unwrap(), 12345);

        let float_str = string_from_bytes_impl(b"3.14159".as_ptr(), 7).unwrap();
        let parsed = string_parse_float_impl(float_str).unwrap();
