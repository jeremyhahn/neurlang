...
pub fn register_json_extensions(registry: &mut ExtensionRegistry) {
    // json_parse(ptr, len) -> handle
    registry.register(
        "json_parse",
    ...
    // json_stringify(handle, ptr, max_len) -> bytes_written
    registry.register(
        "json_stringify",
    ...
    // json_get(handle, key_ptr, key_len) -> new_handle
    registry.register(
        "json_get",
    ...
    // json_set(handle, key_ptr, key_len, value_handle) -> 0
    registry.register(
        "json_set",
    ...
    // json_get_type(handle) -> type constant
    registry.register(
        "json_get_type",
    ...
    // json_array_get(handle, index) -> new_handle
    registry.register(
        "json_array_get",
    ...
    // json_array_push(handle, element_handle) -> 0
    registry.register(
        "json_array_push",
    ...
    // json_object_keys(handle) -> vec_handle
    registry.register(
        "json_object_keys",
...
mod tests {
    ...
    fn test_json_parse_object() {
        ...
        // Get a key
        let name_handle = json_get_impl(handle, b"name".as_ptr(), 4).unwrap();
        assert_eq!(json_get_type_impl(name_handle).unwrap(), json_type::STRING);

        // Free
        json_free_impl(handle).unwrap();
        json_free_impl(name_handle).unwrap();
    }
    ...
    fn test_json_parse_array() {
        ...
        // Get element
        let elem = json_array_get_impl(handle, 2).unwrap();
        assert_eq!(json_get_type_impl(elem).unwrap(), json_type::NUMBER);

        // Free
        json_free_impl(handle).unwrap();
        json_free_impl(elem).unwrap();
    }
    ...
    fn test_json_stringify() {
        ...
        let len = json_stringify_impl(handle, buffer.as_mut_ptr(), buffer.len()).unwrap();
    ...
    fn test_json_build() {
        // Create object
        let obj = json_new_object_impl();

        // Create array
        let arr = json_new_array_impl();

        // Check types
        assert_eq!(json_get_type_impl(obj).unwrap(), json_type::OBJECT);
        assert_eq!(json_get_type_impl(arr).unwrap(), json_type::ARRAY);

        // Free
        json_free_impl(obj).unwrap();
        json_free_impl(arr).unwrap();
    }
}
